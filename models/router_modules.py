...
class RouterInterface(Node):
    ...
    def process_ip_packet(self, ip_packet, source_mac):
        ...
        print(f"Interface {self.mac_address} received IP packet from 0x{ip_packet.source_ip:02X} to 0x{ip_packet.dest_ip:02X}")
        ...
        if ip_packet.dest_ip == self.ip_address:
            ...
            if self.router:
                ...
                if outgoing_interface:
                    print(f"  Route found: forward via interface {outgoing_interface.mac_address}")
                    ...
                    if ip_packet.dest_ip in outgoing_interface.arp_table:
                        ...
                        print(f"  Forwarding packet to 0x{ip_packet.dest_ip:02X} (MAC: {dest_mac}) via interface {outgoing_interface.mac_address}")
                        outgoing_interface.send_ip_packet(ip_packet, dest_mac)
                    else:
                        print(f"  ERROR: No ARP entry for 0x{ip_packet.dest_ip:02X} on interface {outgoing_interface.mac_address}")
                else:
    ...
    def send_ip_packet(self, ip_packet, dest_mac):
        ...
        print(f"Interface {self.mac_address} sending IP packet from 0x{ip_packet.source_ip:02X} to 0x{ip_packet.dest_ip:02X}")
